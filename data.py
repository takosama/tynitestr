...
def preprocess_corpus(corpus_path: Path, out_suffix: str = ".clean.csv") -> Path:
    ...
    for chunk in pd.read_csv(
        ...
        if target_col not in chunk.columns:
            ...
            if obj_cols:
                target_col = max(obj_cols, key=lambda c: chunk[c].astype(str).str.len().mean())
            else:
                target_col = chunk.columns[0]
        chunk[target_col] = chunk[target_col].astype(str).map(_clean_text_for_ime)
        chunk.to_csv(out_path, sep=sep, index=False, mode=("w" if first else "a"),
...
def build_memmap_tokens(corpus_path: Path, tokenizer: ByteBPETokenizer,
                        tok_bin=TOK_BIN, off_bin=OFF_BIN, chunksize=100_000):
    ...
    with open(tok_bin, "wb") as f_out:
        if ext in [".csv", ".tsv"]:
            ...
            text = unicodedata.normalize("NFKC", corpus_path.read_text(encoding="utf-8"))
...
class MemmapNexTokDataset(Dataset):
    ...
    def __getitem__(self, _):
        ...
        span = torch.from_numpy(self.tok[start:start+self.window+1].astype(np.int64))
        x = span[:-1]  # 長さ=WINDOW
...
def detect_text_col(path: Path) -> str:
    ...
    if ext in [".csv", ".tsv"]:
        df_head = pd.read_csv(path, sep="\t" if ext==".tsv" else CSV_SEP,
                              dtype=str, on_bad_lines="skip", nrows=2000)
