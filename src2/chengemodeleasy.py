...
def _infer_ckpt_vocab(sd: dict) -> int:
    ...
    raise ValueError(
        "state_dict に wte.weight / lm_head.weight が見つからないため、vocab_sizeを推定できません。"
    )


def convert_checkpoint_to_executable_model(
    pt_path: str, out_path: str = None, force_vocab_size: int | None = None
):
    """
    ...
    if "model" not in ckpt:
        raise ValueError(
            f"{pt_path} はbundle形式ではありません（'model'キーが無い）。weights-onlyは変換不要です。"
        )
    ...
    if target_vocab == ckpt_vocab:
        ...
        if target_vocab < ckpt_vocab:
            raise ValueError(
                f"force_vocab_size={target_vocab} < ckpt_vocab={ckpt_vocab} はサポート外（縮小は不可）。"
            )
    ...
    print(
        f"[convert] missing={len(ret.missing_keys)}, unexpected={len(ret.unexpected_keys)}"
    )
